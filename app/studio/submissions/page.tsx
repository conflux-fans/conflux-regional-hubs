...
export default async function SubmissionsPage() {
  ...
  if (!(await canEdit(user))) return <main className="studio-access"><p className="v2-kicker">QUESTIONNAIRE SUBMISSIONS</p><h1>Editor access required.</h1><p>You are signed in as {user.email}, but this account is not on the regional editor list.</p><Link href={chatGPTSignOutPath(returnTo)}>Sign out</Link></main>;

  const submissions = await getRegionalBriefSubmissions();
  return <main className="studio-page submissions-page">
    <header className="studio-header"><div><p className="v2-kicker">PRIVATE COORDINATOR INBOX</p><h1>Regional<br />submissions.</h1><p>Review every questionnaire and forward its generated prompt to the developer.</p></div><nav><Link href="/questionnaire?region=africa">Open public questionnaire ↗</Link><Link href="/studio?region=africa">Website manager</Link><Link href={chatGPTSignOutPath(returnTo)}>Sign out</Link></nav></header>
