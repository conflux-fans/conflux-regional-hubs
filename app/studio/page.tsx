...
import { canEdit } from "../lib/editor-auth";
...
async function ProtectedStudio({ regionKey }: { regionKey: RegionKey }) {
  ...
  if (!(await canEdit(user))) {
    return <main className="studio-access"><p className="v2-kicker">REGIONAL STUDIO</p><h1>Editor access required.</h1><p>You are signed in as {user.email}, but this account is not on the regional editor list.</p><Link href={chatGPTSignOutPath(returnTo)}>Sign out</Link></main>;
  }
  ...
  return (
    <main className={`studio-page region-${region.key}`} style={{ "--region-accent": region.accent, "--region-secondary": region.secondary, "--region-tertiary": region.tertiary, "--region-on-accent": region.onAccent, "--region-surface": region.surface } as React.CSSProperties}>
      <header className="studio-header"><div><p className="v2-kicker">REGIONAL WEBSITE STUDIO / {region.code}</p><h1>Create once.<br />Manage easily.</h1><p>Signed in as {user.displayName}</p></div><nav><Link href="/studio/submissions">Questionnaire submissions</Link><Link href={`/?region=${region.key}`}>View current website ↗</Link><Link href={`/studio?region=${region.key === "africa" ? "korea" : "africa"}`}>Switch region</Link><Link href={chatGPTSignOutPath(returnTo)}>Sign out</Link></nav></header>
