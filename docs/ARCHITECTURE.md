...
- Staking copy and presentation remain regional, while chain ID, proxy-slot checks, units, minimal ABI, transaction state, and wallet calls stay in the shared `app/lib/staking/` core. Pool targets are deployment-configured, and the feature flag fails closed before any wallet write request.
