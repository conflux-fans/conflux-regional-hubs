...
> 更新日期：2026-08-21
> 需求来源：`docs/staking.md`
> 合约接口：`docs/IPoSPool.json`

本文档在原始需求基础上补充可实施的交互、数据映射、边界条件、验收标准和发布检查。原始需求与 ABI 是输入材料，不由本文档替代或修改；发生冲突时，先核对链上合约，再由产品/技术负责人更新结论。

## 1. 目标与产品原则

一期在 `/stake` 提供 Conflux Core Space PoS 矿池的只读数据和四类用户交易：质押、赎回、提取本金、领取收益。
...
页面只帮助用户调用既有矿池合约，不托管私钥、不代签交易、不保存助记词，也不承诺固定收益。所有写操作必须由 Fluent 钱包向用户展示并确认。

## 2. 一期范围

### 2.1 包含

- Conflux Core Space 主网，network ID `1029`；
- Fluent 钱包连接、断开、账户切换和网络切换检测；
- 当前账户的 Core Space CFX 余额；
- 矿池总质押量、质押人数和最近 APY；
- 当前用户的质押概览、锁定队列、解锁队列和收益概览；
- `increaseStake`、`decreaseStake`、`withdrawStake`、`claimAllInterest` 四条交易流程；
- 交易前校验、费用/抵押估算、钱包签名、回执等待、结果反馈和数据刷新；
- 桌面端和移动端的响应式展示；
- 功能开关、网络/地址白名单和上线前合约校验。

### 2.2 不包含

- Conflux eSpace、MetaMask 或跨空间桥；
...
| 项目 | 一期值 | 要求 |
| --- | --- | --- |
| Space | Core Space | 禁止把地址或交易发送到 eSpace |
| 网络 | Conflux 主网 | network ID 必须为 `1029` |
| 钱包 | Fluent | 使用其注入的 Conflux provider |
| SDK | `js-conflux-sdk` | 金额和整数全程使用无精度损失的类型 |
| 合约地址 | `cfx:acdj1y1r00mzvuw9s831rj1t5amst2405jv582syu0` | 必须由开发者配置并进入允许列表 |
| ABI | `docs/IPoSPool.json` | 运行时代码应抽取最小用户 ABI，不向 UI 暴露管理员方法 |
| CFX 最小单位 | Drip | `1 CFX = 10^18 Drip` |
| 票数单位 | votePower | `1 votePower = 1000 CFX` |

合约地址是 Core Space base32 地址，比较时必须先规范化到同一 network ID 和地址类型；不要用大小写或字符串截断结果做安全判断。

### 3.1 上线前链上校验

2026-08-21 的只读检查显示：该地址在 Core Space 主网有合约代码、为 EIP-1967 代理、`_poolRegisted()` 为 `true`，链上 `VERSION()` 为 `1.8.0`。仓库当前上游源码已出现 `1.9.0`，因此不能仅凭仓库 HEAD 推断线上实现。

每次发布到生产前必须重新验证：

- RPC 返回的 network ID 为 `1029`；
- 代理地址代码非空，代理实现地址与已审核记录一致；
- `VERSION()`、`_poolRegisted()`、`_poolLockPeriod()`、`_poolUnlockPeriod()` 可正常读取；
...
- 质押金额不得超过钱包余额扣除预计交易费用和存储抵押后的可用金额；
- 赎回金额不得超过 `userSummary.locked * 1000 CFX`；
- 提取金额不得超过 `userSummary.unlocked * 1000 CFX`；
...
| 矿池是否可用 | `_poolRegisted()` | `false` 时禁用全部交易 |
...
- 当前 epoch/block number：判断队列是否到期。
...
| 状态 | 页面行为 |
| --- | --- |
| 未安装 Fluent | 展示安装引导；不发起连接请求 |
| 已安装、未连接 | 允许看矿池数据；用户数据占位；显示“连接 Fluent” |
| 正在连接 | 禁用重复点击；显示等待钱包响应 |
| 用户拒绝连接 | 保持未连接状态并给出可重试提示 |
| 已连接且 network ID = 1029 | 读取用户数据并允许交易 |
| 已连接但网络错误 | 显示目标网络；禁用交易；提供钱包支持时的切网操作 |
| 账户切换 | 清除旧账户缓存和输入，立即以新账户重读 |
| 网络切换 | 清除合约实例和用户缓存，重新校验后再启用交易 |
| 钱包断开/权限撤销 | 清空用户数据并回到未连接状态 |

地址展示为短格式，但复制按钮复制完整 Core Space 地址。不得把 eSpace 十六进制地址作为一期默认展示或交易目标。
...
4. 调用 `increaseStake(votePower).estimateGasAndCollateral({ from, value })`；
5. 向 Fluent 发送目标为允许列表合约的交易；
...
6. 到期后金额转入“可提取本金”，不会自动回到钱包。

若未来启用了 Voting Escrow，合约还可能因治理锁仓限制实际可赎回量。遇到 `Locked is not enough` 时重新读取状态并提示部分 CFX 可能被锁定，不得自动反复提交。

### 8.3 提取本金（Withdraw）

一期采用“全部提取”交互：按钮金额为 `userSummary.unlocked * 1000 CFX`，调用 `withdrawStake(userSummary.unlocked)`。按钮在可提取票数为零时禁用。

交易确认后刷新钱包余额、用户概览和解锁队列。成功文案为“本金已提取至当前钱包”。如果后续增加部分提取，输入仍须为 `1000 CFX` 的整数倍，并转换为 votePower 后调用。
...
ABI 中的 `claimInterest(amount)` 参数单位是 Drip，可留给后续“部分领取”，一期 UI 不暴露，避免把 CFX 数值误作为 Drip 发送。
...
- 获得交易哈希后立刻展示并提供 Core Space 区块浏览器链接；
- 不以拿到哈希作为成功，必须读取交易回执；
- 回执 `outcomeStatus` 成功后才显示成功；失败时展示失败并重读链上状态；
...
| 来源/错误 | 用户提示 | 后续动作 |
| --- | --- | --- |
| Fluent 未安装 | 未检测到 Fluent | 提供官方安装入口 |
| 用户拒绝连接或签名 | 操作已取消 | 不记录为失败交易，可重试 |
| 网络不是 1029 | 请切换到 Conflux Core 主网 | 禁用交易 |
...
| `Unlocked is not enough` | 当前可提取本金不足 | 重读用户状态 |
| `No claimable interest` / `Interest not enough` | 当前没有足够可领取收益 | 重读收益 |
| `Pool is not registed` | 矿池当前不可用 | 全局禁用写操作 |
...
- 合约地址、network ID 和允许调用的方法由开发者控制，不接受 URL、CMS 或用户输入覆盖；
- 前端最小 ABI 只包含本规格的读方法、四个写方法和相关事件；
- 交易发送前再次校验 network ID、`to`、method selector、参数和值；
...
- 页面明确披露：第三方矿池合约和 PoS 均有智能合约、验证节点、锁定周期及协议处罚风险；
...
lib/staking/constants.ts           network、单位和经审核的合约允许列表
lib/staking/abi.ts                 从 artifact 固化的最小用户 ABI
lib/staking/amounts.ts             Drip/CFX/votePower 的纯函数转换
lib/staking/provider.ts            Fluent provider 发现、连接、事件订阅
lib/staking/pos-pool.ts             只读调用、估算和四个写调用的适配器
lib/staking/transactions.ts        回执轮询和状态规范化
lib/staking/errors.ts              provider/revert 错误到产品文案的映射
config/region-types.ts             Stake 可见文案/展示配置的类型
config/regions.ts                  各区域的文案与展示变体
...
NEXT_PUBLIC_CONFLUX_NETWORK_ID=1029
NEXT_PUBLIC_CONFLUX_RPC_URL=https://main.confluxrpc.com
NEXT_PUBLIC_STAKING_CONTRACT=cfx:acdj1y1r00mzvuw9s831rj1t5amst2405jv582syu0
```

生产构建中 `STAKING_ENABLED=true` 时，配置缺失、network ID 非 1029 或地址不在代码允许列表都应使构建失败或功能保持禁用。RPC URL 是公开读取端点，不得放私钥。
...
- 队列到期判断和预计时间；
- 合约错误映射、地址/network 允许列表校验。

### 14.2 适配器测试

- `increaseStake(3)` 必须携带精确的 `3000 CFX` Drip value；
- 其他三个写方法 value 必须为零；
- 质押、赎回和提取参数均为 votePower，收益部分领取参数为 Drip；
- Fluent 拒绝、RPC 超时、estimate 失败、回执成功/失败/迟到；
- 账户或网络切换时旧请求不污染新状态；
- 重载队列方法选择正确。
...
2. 运行网络、代理实现、版本、ABI 和只读 smoke test；
3. 在受控环境完成 Fluent 人工小额端到端验证；
4. 只对内部/指定区域开启；
...
2. Core Space 区块浏览器交易链接采用哪个官方/社区域名；
3. Fluent 的最低支持版本及实际注入 provider API；
...
- 原始需求：`docs/staking.md`
- 本地 ABI：`docs/IPoSPool.json`
...
- Core Space 概览：<https://doc.confluxnetwork.org/docs/core/Overview/>
- Core Space RPC 端点与 network ID：<https://doc.confluxnetwork.org/docs/core/conflux_rpcs/>
- Fluent 与钱包说明：<https://doc.confluxnetwork.org/docs/general/conflux-basics/wallets/>
- `js-conflux-sdk`：<https://github.com/Conflux-Chain/js-conflux-sdk>
- 交易回执：<https://doc.confluxnetwork.org/docs/core/build/json-rpc/rpcs/cfx_getTransactionReceipt/>
